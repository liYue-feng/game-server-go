...
func validateCombatResult(req *protocolpb.CombatResultReq, cfg *CombatConfig) error {
	...
	// 分数上限检查：每级有最大合理分数
	maxScore := int64(cfg.MaxScorePerLevel) * int64(req.DungeonLevel)
	if req.Score < 0 || req.Score > maxScore {
		return fmt.Errorf("分数异常: score=%d, max=%d", req.Score, maxScore)
	}

	// 击杀数上限检查
	dungeonCfg := GetDungeonConfig(int(req.DungeonLevel))
	if dungeonCfg != nil {
		maxKills := int32(float64(dungeonCfg.RoomCount) * float64(dungeonCfg.EnemyDensity) * cfg.MaxKillsMultiplier)
