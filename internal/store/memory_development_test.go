...
func TestMemoryDevelopmentStoreCreatesIndependentPlayerCopies(t *testing.T) {
	...
	player := &Player{OpenID: "dev:alice", Level: 1}
	...
	second := &Player{OpenID: "dev:bob", Level: 0}
	if err := developmentStore.CreatePlayer(second); err != nil {
		t.Fatalf("CreatePlayer(second) error = %v", err)
	}
	if second.ID != 2 || second.Level != 1 {
		t.Fatalf("CreatePlayer(second) = ID %d, level %d; want ID 2, level 1", second.ID, second.Level)
	}
	if err := developmentStore.CreatePlayer(&Player{OpenID: "dev:alice", Level: 1}); err == nil {
	...
	if err := developmentStore.UpdatePlayer(&Player{ID: 999, OpenID: "dev:missing", Level: 1}); !IsNotFound(err) {
...
func TestMemoryDevelopmentStoreConcurrentPlayerCreation(t *testing.T) {
	...
	for i := 0; i < playerCount; i++ {
		go func(index int) {
			...
			player := &Player{OpenID: fmt.Sprintf("dev:user-%d", index), Level: 1}
